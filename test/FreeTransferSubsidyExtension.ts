...
describe('FreeTransfersSubsidyExtension', () => {
  const fixture = async () => {
    ...
    const dailyLimit = 10;
    await extension.connect(owner).registerToken(await erc20.getAddress(), dailyLimit);
  ...
  it('Changing limit resets bucket to full', async function () {
    const erc20Address = await this.erc20.getAddress();
    const newLimit = 20;
    await this.extension.connect(this.owner).setFreeTransfersDailyLimit(erc20Address, newLimit);
  ...
  it('Setting limit to 0 stops sponsorship', async function () {
    const erc20Address = await this.erc20.getAddress();
    await this.extension.connect(this.owner).setFreeTransfersDailyLimit(erc20Address, 0);
  ...
  describe('Token registration', () => {
    it('Contract owner can register a token', async function () {
      ...
      await expect(this.extension.connect(this.owner).registerToken(tokenAddress, 50))
        .to.emit(this.extension, 'TokenRegistered')
        .withArgs(tokenAddress, 50);
    });

    it('Non-owner cannot register a token', async function () {
      const token = await ethers.deployContract('TestingERC20', []);
      await expect(
    ...
    it('Rejects duplicate token registration', async function () {
      await expect(
        this.extension.connect(this.owner).registerToken(await this.erc20.getAddress(), 50),
  ...
  describe('Leaky bucket boundary conditions', () => {
    ...
    it('max uint96 limit after 400 days returns exact limit (no uint104 silent truncation)', async function () {
      ...
      const token = await ethers.deployContract('TestingERC20', []);
      const tokenAddress = await token.getAddress();
      await this.extension.connect(this.owner).registerToken(tokenAddress, maxUint96);
    ...
    it('track() is a no-op when bucket is empty', async function () {
      ...
      const trackingId = ethers.zeroPadValue(
        ethers.toBeHex((0xf7n << 248n) | BigInt(erc20Address)),
        32,
      );
    ...
    it('reducing dailyLimit resets bucket to the new lower limit', async function () {
      ...
      const lowerLimit = 3;
      await this.extension.connect(this.owner).setFreeTransfersDailyLimit(erc20Address, lowerLimit);
  ...
  describe('Removing tokens', () => {
    it('Contract owner can remove a token', async function () {
      const erc20Address = await this.erc20.getAddress();
      await expect(this.extension.connect(this.owner).removeToken(erc20Address))
    ...
    it('Rejects removal of non-registered token', async function () {
      const token = await ethers.deployContract('TestingERC20', []);
      await expect(
        this.extension.connect(this.owner).removeToken(await token.getAddress()),
    ...
    it('chooseFund returns NONE for removed token', async function () {
      await this.extension.connect(this.owner).removeToken(await this.erc20.getAddress());
