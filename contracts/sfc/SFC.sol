...
contract SFC is OwnableUpgradeable, UUPSUpgradeable, Version {
    ...
    function distributeExtraReward(uint256 epoch, bool withBurn) external payable {
        ...
        // log the burned amount; a dust can always be burned due to int math rounding the share down
        if (amountDistributed < msg.value) {
            emit BurntNativeTokens(msg.value - amountDistributed);
        }
    }
